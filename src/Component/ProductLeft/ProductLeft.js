...
componentDidUpdate(oldProps) {
  ...
  productdelet = (productdata)=>{
      let ProductList = this.state.ProductList
      let filterdata = ProductList.filter(item=>item.ProductId !== productdata)
      let acctdata = JSON.parse(localStorage.getItem('accountdata'));
      acctdata['productsPage']['products'] = filterdata
      localStorage.setItem('accountdata', JSON.stringify(acctdata));
    this.setState({
  ...
  displayProducts = (data) => {
    ...
    return (   
      <tr className={classes.thead}>
                    <th className={classes.thspc}><input type="checkbox"  value={data.ProductId}/> </th>
                    <td className={classes.tm_product_name}>{data.name}</td>
                    <td>{data.unitSold}</td>
                    <td>{data.stock}</td>
                    <td>{data.expireDate}</td>
                    <td>
                      <a href="#" onClick={()=>this.productdelet(data.ProductId)} className={classes.tm_product_delete_link}>
        ...
        return (
            <div class="col-sm-12 col-md-12 col-lg-8 col-xl-8 tm-block-col">
               ...
               <Link to='/product/addnew'
                  ...
                  <button class="btn btn-primary btn-block text-uppercase" onClick={this.deletselectproduct}>
